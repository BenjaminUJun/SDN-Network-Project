...
class Network_Aware(app_manager.RyuApp):
    ...
    def update_link_delete(self,ev):
	link = ev.link
	src=link.src
	dst=link.dst
	if (src.dpid,dst.dpid) in self.link_to_port.keys():
		del self.link_to_port[(src.dpid,dst.dpid)]
	#print self.link_to_port
	#print ev
	self.graph[src.dpid][dst.dpid]=float('inf')
	#print 'pre graph'
	#print self.pre_graph
	#print 'update graph'
	#print self.graph
